...
public class AuctionController {
    ...
    public ResponseEntity<List<AuctionResponse>> getAuctionsByUser(@PathVariable Long userId) {
        List<AuctionResponse> auctions = auctionService.getAuctionsByUser(userId);
        return ResponseEntity.ok(auctions);
    }
    
    @GetMapping("/{auctionId}/countdown")
    public ResponseEntity<?> getCountdownStatus(@PathVariable Long auctionId) {
        try {
            CountdownStatusResponse response = auctionService.getCountdownStatus(auctionId);
            return ResponseEntity.ok(response);
        } catch (RuntimeException e) {
            return ResponseEntity.badRequest().body(e.getMessage());
        }
